...
| Rescind feeds the generator | **LOCKED** | A rescinded assignment is not a delivered one. School work with **no live row anywhere** returns to the pending walk at its position in the course, so the pacing budget re-places it and pushes later work out; a rescinded **chore** occurrence is never re-proposed, because its identity contains its date and the next day already has its own. One new parent-only read route (`GET /api/assignments/reassignable`), no schema change, no write. See `TDS_Slice_Rescind_Regeneration.md`. |
