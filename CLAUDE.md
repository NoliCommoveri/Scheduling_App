...
| Rescind feeds the generator | **LOCKED** | A rescinded assignment is not a delivered one. School work with **no live row anywhere** returns to the pending walk at its position in the course, so the pacing budget re-places it and pushes later work out; a rescinded **chore** occurrence is offered again **only for its own date** — the one date its id can name — and never migrates to a later day, so a missed chore day stays missed while a pulled-back fortnight can be re-assigned a day at a time. One new parent-only read route (`GET /api/assignments/reassignable`), no schema change, no write. See `TDS_Slice_Rescind_Regeneration.md`, amended §2.2/§2.2a the day it shipped. |
...
| Course Structure Expander | **LOCKED** | A Management App module, not a fourth app: `#/expander` under the Library hub. Turns a curriculum counts sheet and a trimmed-PDF page map into a proposal in the bulk importer's own 12-column format, filling every mechanical column (course code, Lesson codes and order, Activity types, titles, tiers, durations) and leaving one thing to the LLM pass — splitting each Lesson's page range into named sections. **Nothing is narrowed and no rule bends**: no schema, no migration, no Worker route, no credential class, and the module writes nothing at all. Lessons and Activities still enter only through `Courses.importActivitiesCsv()`. `.xlsx` is read with the platform's own `DecompressionStream`, so the no-build-step rule holds. See `TDS_Slice_Course_Structure_Expander.md`. |
...
| `docs/TDS_Slice_Rescind_Regeneration.md` | Rescind feeds the generator: pulled-back school work returns to the walk, a rescinded chore day stays spent. |
| `docs/TDS_Slice_Course_Structure_Expander.md` | The Course Structure Expander (`#/expander`): counts sheet + PDF page map → a proposal in the bulk importer's own 12-column format. Management App only; reads four stores, writes nothing. |
...
| 2.10 | 2026-08-28 | **A gap in the generator, closed with one read — no narrowing, no rule bent.** Ray reported that rescinded school tasks never come back: he assigns daily, and when a child misses a lesson he wants to rescind it and regenerate so today re-places it and pushes today's work out. Propose decided what was still owed from the **local** `generationLog`, which is written at Commit and never again — so a rescind (a Worker call stamping `rescinded_at` on the D1 row) left the log saying `sent` and the walk stepped past that lesson permanently. `loadCommittedKeys` already carried the reason in its own comment — *"those items must become assignable again"* — and nothing had ever been wired to it. §I.A's Management App Data Flow cell gains `GET /api/assignments/reassignable`, a parent-only read naming the school activities with **no live row anywhere**; Propose drops them from its sent set so the walk owes them again, and stops reproducing any decision D1 says was pulled back. §VII gains a row and §VIII a file reference. Ray drew the asymmetry himself, then corrected it within the hour after the first build made a day un-assignable (slice §2.2a): **a rescind un-assigns for both subjects**, and the chore half is that an occurrence is re-offered **only for its own date** — never carried onto another day. A missed chore day therefore stays missed, while a rescinded fortnight can be re-assigned one day at a time, which is what he was doing. Nothing widens: no schema change, no migration, no write of any kind, no new credential class, and the natural-key guard (Online Revamp §6.6.2) is untouched. Authorized by Ray in-session, 2026-08-28. See `docs/TDS_Slice_Rescind_Regeneration.md`. |
| 2.11 | 2026-08-31 | **A new Management App module, and nothing else — no narrowing, no rule bent.** Ray rebuilt a lost standalone HTML page that sat at step 3 of a four-step curriculum-import chain, and asked for it in the repo with the manual columns automated away. It went into the Management App rather than becoming a fourth app because the three things being automated — `courseCode`, `lessonCode`/`lessonOrder`, and the `activityType`/`difficultyTier` vocabularies — are all records only that app holds; a standalone page would have to ask for them to be typed and would keep a second copy of the seeds to drift. §VII gains a row and §VIII a file reference. **§I.A's three-app isolation is untouched** — this is a module inside an existing app, sharing no code with the Child or Wall apps, and a fourth app would have meant rewriting §I.A and §I.B. No schema change, no migration, no Worker route, no credential class, and **the module writes nothing**: it reads `courses`, `lessons`, `activityTypes` and `tiers`, and produces a download. Lessons and Activities still enter through exactly one door, `Courses.importActivitiesCsv()`, which the expander deliberately does not shortcut — its output is incomplete by design (blank `pdf` titles) and must leave for the LLM pass regardless. Reading `.xlsx` uses the platform's own `DecompressionStream('deflate-raw')`, so §0's vanilla-JS no-build-step rule holds with no library and no vendored code. Authorized by Ray in-session, 2026-08-31, who chose the placement from a recommendation. See `docs/TDS_Slice_Course_Structure_Expander.md`. |
