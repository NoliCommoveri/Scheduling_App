...
*(Amended by FR-13, 2026-08-13: inside a course collapse the section header already names the course, so the card omits this label — the same move the Lesson Recipe slice §9.1 made when `lessonTitle` became a group header. Outside one — the School filter view, the Subjects view, an item with no course — FR-12 is unchanged.)*

**FR-13 — Course and chore collapse (Today and Completed).** In the **Today** view, the School half of each block renders as one **collapsible section per course**, labelled with the course name, and the Chore half of that block renders as **one collapsible section** labelled "Chores". In the **Completed** view — which has no blocks — completed School items render as one collapsible section per course and every completed Chore, including a shared chore a sibling claimed (Shared Chores §6.2), renders in one "Chores" section.

- **Closed by default.** Opening the app shows the day as a short list of names, not a wall of cards. A section header carries a **count** of the items inside, so a closed section still says how much is in it.
- **Expansion is remembered for the session, not stored.** A section a child opens stays open across every re-render — a completion, a reorder, a sync poll — and is *not* persisted: reopening the app starts from closed again. Expansion state is display-only and is never written to an assignment row or the outbox.
- **Sections are independent.** The same course split across two blocks is two sections that open and close separately, as is the same course on Today and on Completed.
- **Grouping changes nothing about order.** Within a section, FR-1's ordering is untouched — lesson sub-grouping (Lesson Recipe §9.2) and the within-lesson sort are exactly what they were, and FR-1's category split still puts every course above the block's chores.
- **The sections themselves are ordered by FR-1's position axis** — a section sits where its earliest item sits, so the course whose first item comes first is the first section. *(Added 2026-08-25 by `TDS_Slice_Subject_Order_Grouped_Review.md` §2.7, Phase 4 — design only, unbuilt at the time of writing. Until then this is undefined in the code and resolves to the order the rows happen to come out of the cache, which is assignment-id order and therefore arbitrary. Once defined, a day's sections follow the order the parent's screens use — Management Module 08 FR-17 — because that order is what `sort_order` carries. A child's own reorder (FR-4) still wins within a section, and can move a section when it changes that section's earliest position; the parent's order is the starting order, not a lock.)*
- **Items with no course** collapse into one trailing section, named for the child ("Other school work"), never left loose beside the sections and never given an invented course name.
- **An emptied section disappears with its items.** Completing the last item in a course removes that course from Today entirely — header, count and all — because the section exists only where FR-1 puts items in it. A block whose sections have all emptied renders nothing, per FR-16's rule for blocks.
- Reorder (FR-4) and per-item block moves (FR-5) are unchanged inside an open section.

**FR-14 — Move a whole course between blocks.** A course section's header on the **Today** view carries a block picker that moves **every item in that section** to the chosen block in one action. It writes exactly what FR-5's per-item picker writes — the child's block override, per item — and nothing else: no due date, required status, completion state or sort position changes, and each item keeps its position within its new group. The per-item picker (FR-5) remains available inside the section. The "Other school work" section and the "Chores" section have no such control — a course is the only grouping a child can mean as a unit. A section moved this way stays open under its new block.
...
17. Importing `packet_sample.json` on 2026-09-14 produces a Today view whose morning block contains, in order: the two Saxon Math activities (School), then "Unload dishwasher" (Chore); whose afternoon block contains, in receipt order, "Timeline Practice" then "Draw a Still Life" (both School); and whose Events section contains "Piano recital" — with no evening or night block rendered. *(As of FR-13 each of those groups is reached by opening its section; the membership and the order are what this criterion tests, not whether the cards are on screen before a tap.)*
18. **(FR-13)** Today opens with every course section and every chore section closed, each header showing the count of items inside it. Opening a section reveals exactly FR-1's items in FR-1's order, lesson headers included; the cards inside a course section do not repeat the course name (FR-12 as amended).
19. **(FR-13)** A section a child opens is still open after they complete an item in it, reorder inside it, or a sync poll redraws the plan — and is closed again after the app is restarted. Two blocks holding the same course open and close independently, and so do Today and Completed.
20. **(FR-13)** Completing the last item of a course removes that course's section from Today; nothing renders an empty section, an empty count, or an orphaned header.
21. **(FR-14)** Choosing a new block on a course section header moves every item in that section to that block at once, leaves everything but their block override untouched, and leaves the section open under its new block. Doing it to a course whose items were split across two blocks moves only the section that was chosen.
