...
> **Mechanism updated 2026-08-12 — the requirement stands, what enforces it does not.** "Never double-assigns" was satisfied by the packet: a re-run produced an identical file and the child *replaced* its plan on import, so a duplicate could not survive the transport. `TDS_Slice_Online_Revamp.md` repealed that transport for an insert-only D1 table and did not rebuild the guarantee, so re-running a covered range put every item on the child's plan a second time. It is now enforced on the natural key `(child_id, date, kind, source_id)` — see Revamp §6.6, which is authoritative for this FR. The log is still what keeps *proposal* content stable across runs; it is no longer what keeps the *assignment* unique.
...
- **`sortOrder` is unchanged in definition and follows the order above**, because it has always been derived from position within the kind's band (FR-14). One consequence is intended, and arrives in two parts: `sortOrder` carries the standing subject order to the child immediately, and the child's *course sections* follow it once Child Module 03 FR-13 defines their order (`TDS_Slice_Subject_Order_Grouped_Review.md` §2.7 — a Child App change, phased separately, because until it lands the child groups its courses in the order its cache returns rows). One is a known limit: on a day committed across two passes, an already-live row keeps the number D1 gave it (Revamp §6.6), so a row inserted ahead of it on a later pass can carry a number that disagrees with the review screen's ordering. The Assignments view's Sort order field is the manual correction; nothing renumbers a live row automatically.
