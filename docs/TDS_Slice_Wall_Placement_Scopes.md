...
**Status:** **PHASES 1–5 BUILT** 2026-08-23 (schema, Worker API, the pure layer, the chore UI, the
block write paths, and the block sheets). **Phase 6 is all that remains**: §8's manual checks on the
tablet and the doc reconciliation in §10. `CLAUDE.md` still records this slice as "authorized,
unbuilt" and stays that way until Phase 6 (§10).
...
**BUILT 2026-08-23 (Phase 4).** The six-row table below is
`SlotsCore.planScopeWrite`, pure and tested — it decides which level a tap writes and which it
clears, and it is the one place the "never clear standing" rule is enforced rather than remembered.
...
**BUILT 2026-08-23 (Phase 5b).** §5.4's long-press sheet (end time + label) becomes the block's
schedule:
...
**BUILT 2026-08-23 (Phase 5b).** §6.2's control lives in the block's own long-press sheet — which works for a skip, since the block
...
**BUILT 2026-08-23 (Phase 5a).** `createSchoolBlock` (`day-ui.js:953`) mints the block, which must now also have weekday rows.
...
Tapping a scope button re-homes the write that just happened: it undoes the drag at the level it
landed on and writes the new level instead. Two taps total for "actually, Fridays only", on the
gesture you were already making, with no dialog in the common path.

**"Undoes the drag" is §6.1's table, not a plain restore** — corrected in the Phase 4 build, where
the first reading of this paragraph produced a button that appeared to do nothing. Going *down* the
chain (standing → an override) nothing is cleared, so the standing row is restored to what it held,
which is the value the toast's own closure already carries (it is what Undo restores,
`day-ui.js`'s `wasAt`). Going *up* it must **clear** the override the drag wrote: a chip whose time
comes from its Friday row, dragged and then re-homed to "Every day", would otherwise keep that
Friday row overriding the standing time just asked for — the chip sits exactly where it was, which
is §7.1's own worst failure arriving through the affordance meant to prevent it. Both halves go
through `planScopeWrite`, so the toast and the sheet cannot disagree about what a scope change means.

**The undo half is written first**, deliberately: two online-required writes with no outbox (§1)
can fail independently, and this order fails visibly (the chip sits where it started) rather than
invisibly (the specific level right, the general one wrong for every other day).

**The scope buttons are withheld on a chore's FIRST placement.** A chip dragged out of the tray has
no level to re-home from, and writing one would leave a dormant override row with the chore still
in the tray — §11.7's deliberately unreachable state, reached by accident. It gets Undo alone.
...
| **4** | Chore UI: §6.1's sheet, §7.1's drag rule, §7.2's toast. **BUILT 2026-08-23**, plus the client wrappers Phase 2's routes never got, the toast's multi-button widening, and §11.6's two loose ends closed. | ~1.5 h |
| **5a** | Block writes: the four client wrappers Phase 2's block routes never got, §6.4's creation default, §7.1 for blocks (the drag writes `spanScope`'s level), and the pure write rules §6.2 needs — `defaultWeekdaysFor`, `planBlockMove`, `todayChoice`, `planDateWrite`. **BUILT 2026-08-23.** ~~§5.3's member fix~~ (done in Phase 3). | ~1 h |
| **5b** | Block sheets: §6.2's schedule sheet including the Today group, §6.3's add-for-today sheet. **BUILT 2026-08-23.** | ~1.5 h |
...
**~9 hours.** Phase 5 split into 5a and 5b during the Phase 5a build, over `CLAUDE.md` §V.A's gate
once the four missing client wrappers and the block drag's scope-awareness were counted — neither
of which §9's one-line row named. **5b must not ship before 5a**, and the reason is the same shape
as the Phase 2/3 dependency: 5a's drag fix is invisible until something creates a weekday or date
span, and 5b's sheet is the thing that creates one. Corrected 2026-08-23 during the Phase 1 build — the first draft of this paragraph
...
**11.10 Dragging a block on a backup day moves the whole term.** Found in the Phase 5a build, and
it is §7.1 working rather than failing. A backup Sunday whose date row carries `occurs: 1` and no
span of its own resolves at `spanScope: 'block'` (§2.2's second table — there is no Sunday weekday
row to read), so the level in force for that span genuinely *is* the block's default, and a drag
there writes it: every scheduled day of the term moves. "You move what you see" gives the honest
answer to "which level put this here" and a surprising answer to "what did I just change."

The chore side answers this with §7.2's toast, which offers the two levels the drag did not write.
Blocks have no equivalent in this design, and adding one is more than a Phase 5 loose end — it needs
the block vocabulary's own three buttons and a re-homing rule (a block's levels do not clear the way
§6.1's do, since the block row is never deleted). **Not built.** §6.2's "Just today…" is the
affordance that fixes it after the fact, which is the same mitigation §7.2 records for the 8-second
toast: discoverable from the sheet, not from the gesture.
...
| Date | Change |
|---|---|
| 2026-08-23 (Phase 5b build) | **The block sheets — the family can now answer "which days does this happen on."** §6.2's schedule sheet (a seven-row weekday checklist, the empty-schedule warning, and §2.2.1's three-radio Today group), §6.3's add-for-today fork on `+ School`, and a span face for editing one level's pair. `TimeCore.weekdayShortName` joins `weekdayName` so the checklist column and the "Every Friday" button name days from one table. `CACHE_NAME` → `wall-display-shell-v20`; 619 green, plus a throwaway Chromium harness driving the real sheet through 31 checks (the §8 manual checks on the tablet are still Phase 6's — a harness with a stubbed `WallApi` proves the wiring, not the wall). **Four findings, none visible from this document alone:** **(a)** **"Use the default" on a weekday is a PUT carrying nulls, and must never be a DELETE** — the one place the block side and the chore side disagree about what an empty override means. `SlotsCore.overrideWrite` maps both-null to a DELETE, because a chore's weekday row that overrides nothing is meaningless; a block's is what SCHEDULES the day (§2.2), so the same reflex here would take the block off Fridays altogether when all the family asked for was Friday at the usual time. **(b)** **§6.2's mock has two buttons where the shipped sheet had three, and the difference is a trap.** The old sheet was Save / Remove / Cancel, with the label and end time waiting for Save. The schedule half cannot wait — each toggle and radio is a complete statement, and the membership picker has written on every checkbox since §5.2 — so the sheet now writes those immediately and keeps ONE commit point: Done, which submits the label and default span only if they changed. Renaming Cancel to Done without that would have silently discarded a relabel. **(c)** **A weekday row's line deliberately ignores the date row.** `resolveBlockSpan(block, weekdayRow, null)` — never the resolved placement — because a skip or a one-off move today says nothing about what Mondays are, and drawing today's time on the Monday row would make a per-date change look permanent. **(d)** **§6.3's Add button is one intent with two verbs**, which is why it goes through `planDateWrite('as-scheduled', …)` rather than writing a row directly: on a day the weekday list excludes it PUTs `occurs: 1`, and on a day it includes — a block skipped earlier — it DELETEs the exception. Also: one overlay with two faces rather than two overlays, so the whole sheet stays a pure function of `blockSheetState` plus the store and no backdrop tap can reach the wrong card. **What is left:** Phase 6 only — §8's manual checks (11, 12, 12a, 13) on the tablet and §10's guardrail amendments. |
| 2026-08-23 (Phase 5a build) | **The block write paths learn the chain, before anything can create a level for them to get wrong.** `school-core.js` gains the write side its resolver has had since Phase 3 — `defaultWeekdaysFor` (§6.4), `planBlockMove` (§7.1), `todayChoice` and `planDateWrite` (§6.2/§2.2.1) — pure and tested for the same reason `planScopeWrite` is: they are decision logic a DOM call site cannot be tested on. `api.js` gains the four wrappers Phase 2's block routes never got and a `weekdays` argument on `postSchoolBlock`. `day-ui.js`: `createSchoolBlock` names its schedule, `moveSchoolBlock` writes `spanScope`'s level through `writeBlockSpan`/`applyOptimisticBlockSpan`, and `removeSchoolBlock` drops the local scope rows the Worker already cascades. `CACHE_NAME` → `wall-display-shell-v19`; 618 green. **Three findings from reading §6.2–§6.4 against the shipped code, none of them in §9's one-line Phase 5 row:** **(a)** **Phase 2's block routes had no client at all** — exactly Phase 4's finding (a) repeating one subject over, and four wrappers rather than the "block UI" §9 called it. **(b)** **The block drag was one commit away from being §7.1's worst failure.** `moveSchoolBlock` wrote `putSchoolBlock({startMin})` unconditionally; the Phase 3 log records that as safe *"because nothing yet creates a weekday or date row with a non-null span"*, and §6.2's sheet is precisely the thing that creates one. Give Friday its own span and the write lands underneath the Friday row: the block does not move. Fixed here rather than in 5b, which is why the phase is split and why **5b must not ship first** — the same dependency shape as Phase 2 before Phase 3. It also had to stop taking its duration from `block.end_min - block.start_min`, or dragging a Friday that runs 8:00–10:30 would silently adopt the default span's length. **(c)** **§6.4's "created scheduled on the day you are standing on" was unimplemented and invisible.** The client sent no `weekdays`, so the Worker's Mon–Fri default applied — correct as a default, wrong as this caller's intent, and on a Saturday it minted a block that was scheduled, invisible, and indistinguishable from a crash to whoever had just tapped `+ School`. New open item **§11.10**: on a backup day with no span of its own, `spanScope` is `'block'`, so a drag there moves the whole term — §7.1 working rather than failing, with no block-side equivalent of §7.2's re-homing toast to soften it. **Not done, and correctly 5b:** the block sheet still edits the block's default span and label only, which stays right under §6.2 — that span is the one it labels "Default". |
| 2026-08-23 (Phase 4 build) | **The chore UI: the writes learn the chain the reads already knew.** §7.1's drag rule (`commitPlacement` writes `resolveChip().scope`'s level, not the standing row unconditionally — a chore leaving the tray still places standing, because §2.1's gate means an override cannot place anything), §7.2's toast (the scope in words, plus the two levels the drag did not write, plus Undo), and §6.1's sheet (a **Starts** stepper above the built **Takes** one; three scope buttons replacing the two-button fork; the level in force marked by being the only filled button). `slots-core.js` gains the write-shape half of the chain — `planScopeWrite`, `levelRow`, `overrideWrite` — pure and tested, because §6.1's table and §4.1's send-your-own-value rule are decision logic that a DOM call site cannot be tested on. Five findings from reading the design against the shipped code, none visible from this document alone: **(a)** **Phase 2's routes had no client.** `api.js` was never given `putSlotWeekday`/`deleteSlotWeekday`, and `putSlotDay` still had its pre-Phase-2 signature with no `startMin` — §9's one-line Phase 4 row folded three API changes into "chore UI" without naming them. Two of its comments were false as well (`deleteSlot` still claimed the server sweeps `wall_slot_days`; `putSlotDay` still claimed `durationMin` is required), both corrected. **(b)** **`toast.js` could show exactly one action button** and §7.2 needs three; `opts.actions` is added beside `opts.action`, which `complete-ui.js` and every other caller keep using. §11.8 predicted the crowding, not the API change. **(c)** **"Use the assigned time" had become a lie in Phase 3.** Widening `isOverridden` to all three levels made the reset button light for a chip whose duration comes from a weekday row, while `submitDurationClear` still cleared only two — and the fix is not a third DELETE: under §4.1's full-row contract, clearing the duration on a level that also carries a **start** override is a PUT keeping its own `start_min`, or Friday's start time goes out with it. Same fork on the date level, which migration 0016 gave a `start_min` too. **(d)** **§7.2's re-homing is §6.1's table, not a plain restore.** "Restores the level the drag wrote to whatever it held before" is right going *down* the chain (standing → an override) and wrong going *up*: dragging a chip whose time comes from its Friday row and then tapping "Every day" must CLEAR that row, since restoring it would leave Friday overriding the standing time just asked for — the chip would sit exactly where it was and the button would appear to do nothing, which is §7.1's worst failure arriving through the affordance meant to prevent it. Both halves now go through `planScopeWrite`. The undo half is ordered first, so a failed pair fails visibly rather than leaving the specific level right and the general one wrong. **(e)** **The scope buttons are withheld on a first placement.** A chore dragged from the tray has no level to re-home from, and writing one would leave a dormant row under §2.1's gate with the chore still in the tray — §11.7's unreachable state, reached by accident. It gets Undo alone. Also closed, both named as Phase 4 loose ends by the Phase 2 build: `applyOptimisticUnplace` no longer sweeps `slotDays` locally where the server keeps them, and every override write now sends that level's own value rather than the resolved one — a drag preserves the target level's `duration_min` verbatim, and the sheet writes the stepper's number only when the family actually moved it, which is test 4b's rule stated as behaviour. `CACHE_NAME` → `wall-display-shell-v18`. §8's Phase 4 additions (§6.1's six-row table, the never-clear-standing property, §4.1's PUT/DELETE fork, `weekdayName`); 614 green. **Not done, and correctly Phase 5:** the block sheet still edits `block.start_min` directly, as the Phase 3 entry records. |
