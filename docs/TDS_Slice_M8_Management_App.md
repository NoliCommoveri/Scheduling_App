...
*Does not cover: Family Event bulk import (still not offered, by design — SRS Module 07 §2.4); Chore bulk import (**offered as of 2026-08-13** — SRS Module 06 Amendment A1 withdrew that module's §2.2 no-bulk-import assumption; the design is `TDS_Slice_Chore_Bulk_Import.md`, which models itself on this slice's §1/§4 but shares no code with it), Master Reporting (M9), Completion Import (M10), Settings/Backup (Module 11, remainder of M8 per the Roadmap — a separate TDS slice), Activity Type CRUD (Module 12, already built at M5).*
