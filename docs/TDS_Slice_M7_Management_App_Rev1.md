...
**Seed at stamp time (FR-1a, added with the default-profile change).** `Pacing.ensureDefaultProfile(instanceId, { hint?, startDate? })` — `children.js` calls it as the last step of `stampCourse`, outside that function's transaction. It returns early when a Profile already exists (`{ record, created: false }`), otherwise reads the Instance's `defaultPacingHint`, hands it to `PacingCore.defaultProfileFields(hint, todayISO())`, and writes the result through the same `saveProfile()` above — so the seeded row goes through the same validation, the same `PAC-{instanceToken}` id, and the same single `put()`, and `pacing.js` remains the sole writer of `pacingProfiles`. Returns `{ record, created: true, source: { days, budget }, summary }` for the stamp confirmation's copy.

**`pacing-core.js` — the hint reader.** New file, alongside `recipe-core.js`: pure, DOM-free, no `Storage` access, `PacingCore.{parseHint, defaultProfileFields, describe}`, covered by `tests/management-pacing-core.test.js`. It is a text reader and nothing more — it holds no storage knowledge and makes no write decisions, which is what keeps the grammar (labelled clauses, day ranges, letter tokens, the per-week exclusion, the sanity bounds) testable without a browser. `management-app/index.html` loads it before `pacing.js`.
