...
**Status:** **BUILT — all three phases landed 2026-08-28.** Authorized by Ray in-session, same day. Reported as "my rescinded school tasks
...
| **Chore occurrence** | a day — sweeping the floor on the 27th | `CHR-{token}-{YYYYMMDD}[-{instance}]`, minted per date (§2.4) | *that day is over.* The 28th has its own occurrence. |
...
Decided: a rescinded assignment is not a delivered assignment. School work whose activity has no
  live row anywhere returns to the pending walk and is re-placed by the ordinary pacing budget,
  pushing later work out. A rescinded chore occurrence is never re-proposed — its day is spent.
Rationale: the school walk owes an activity exactly once and tracks that in one place; a chore's
  identity is per-day and the next day already has its own. Ray stated both halves explicitly
  (§0.1, §0.4). The alternative — a "reassign to today" button on the Assignments view — was
  considered and rejected in §0.5.
...
### 2.2 Chores: the day is spent

No code change. Step 4 already refuses to re-emit an occurrence that has any prior decision
(`decisionItemIds`, `packet.js:435`), and that set is built from every log row for the child
regardless of date or disposition. Because the occurrence id contains the date, yesterday's
suppression cannot reach today's occurrence, which generates normally. The one thing that could have
resurrected a rescinded chore day is reproduction (§0.2), and §2.3 closes it.

This is recorded as a **rule**, not an accident of the id scheme: a chore occurrence is never
returned to a later day by the generator. A parent who genuinely wants yesterday's chore done today
has the Move (§0.5).

### 2.3 Reproduction: a pulled-back decision is not reproduced

Step 2 skips a prior `sent` decision when either is true:
...
5. Propose a range that **contains** the rescinded chore day: the rescinded occurrence is absent
   from that day, and Commit does not re-create it.
6. With the sync token cleared in Settings → Sync, Propose: no item is marked `returned`, the
   already-live notice says the count came from the Generation Log, and nothing is lost.
7. Rescind an activity the parent had previously **Excluded** at Review: it stays out of the
   proposal (§1).
