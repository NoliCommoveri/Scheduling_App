/* Grading Assistant §3.1 — Fry's 1000 most common words, grouped by hundred.
 * Public domain (Edward Fry, "Instant Words"), Worker-only, never served as
 * an asset (management-app/worker/ is excluded — .assetsignore).
 *
 * This is a post-filter, not a prompt input (§0.5): the model reports every
 * suspected misspelling and this file decides which ones a household's
 * rubric may count. Injecting the list into every request would cost ~1300
 * tokens per call for no benefit a deterministic, free, unit-testable filter
 * doesn't already give.
 *
 * Order within each hundred is the list's own canonical order — not
 * alphabetical — since some downstream use may care about frequency rank
 * inside a tier, not just tier membership.
 */

export const FRY_HUNDREDS = Object.freeze([
  Object.freeze([
...
// word (lowercased) -> 1-based frequency rank (1-1000). Built once at module
// load; every lookup after that is a Map hit, not a scan.
const RANK_BY_WORD = new Map();
FRY_HUNDREDS.forEach((hundred, hundredIndex) => {
  hundred.forEach((word, indexInHundred) => {
    RANK_BY_WORD.set(word.toLowerCase(), hundredIndex * 100 + indexInHundred + 1);
  });
});

// Returns the word's 1-1000 frequency rank, or null if it isn't one of the
// 1000. Case-insensitive — the model transcribes as-written capitalization,
// which is not what this list is indexed on.
export function fryRank(word) {
  if (typeof word !== 'string' || !word) return null;
  const rank = RANK_BY_WORD.get(word.toLowerCase());
